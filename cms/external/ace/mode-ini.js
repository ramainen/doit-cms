...
(function() {
    ...
    this.foldingStartMarker = /^\s*\[([^\])]*)]\s*(?:$|[;#])|^\s*container_begin.*$/;
    ...
    this.getFoldWidgetRange = function(session, foldStyle, row) {
        ...
        while (++row < maxRow) {
            ...
            if (/^\s*$/.test(line))
                continue;
            m = line.match(/^\s*\[([^\])]*)]\s*(?:$|[;#])/);
            if (m && typeof(m[1])!='undefined' && m[1].lastIndexOf(startName, 0) !== 0)
                break;
			
			if(firstline.match(/^\s*container_begin.*$/)){
				
				m = line.match(/^\s*container_begin.*$/);
				if (m ){
					cache_cnt++;
				}
				
				
				m = line.match(/^\s*container_end.*$/);
				if (m )      {
